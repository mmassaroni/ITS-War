<?php 
	class Partidas{
		...
		public static function buscarPartida($usuario, $pjElegido){
			$db = new Conexion();
			$consultaPartida = $db->query("select * from partida where estado = 'creando'") or die("ERROR CON LA BD1");
		...
		public static function crearPartida($usuario, $pjElegido){
			$db = new Conexion();
			$db->query("insert into partida(estado) values('creando')") or die("ERROR CON LA BD2");
			$jugadores = new Usus_Pers_Partida();
			$db2 = new conexion();
			$db2->query("insert into usu_pj_partida(partida, numero, usuario, personaje, fuerza, resistencia, vida, energia) values(".mysqli_INSERT_ID($db).", 1, ".$usuario->getid().", ".$pjElegido->getid().", ".$pjElegido->getfuerza().", ".$pjElegido->getresistencia().", ".$pjElegido->getvida().", ".$pjElegido->getenergia().")") or die("ERROR CON LA BD3");
		...
		public static function unirsePartida($usuario, $pjElegido, $regPartida){
			//$jugadores = Usus_Pers_Partida::
			$db = new Conexion();
			$jugadores = new Usus_Pers_Partida();
			$regJugadores = $db->query("select * from usu_pj_partida where partida = ". $regPartida['id']) or die("ERROR CON LA BD4");
